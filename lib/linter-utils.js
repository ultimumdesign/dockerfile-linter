    ...
    for (var i = 0; i < names.length; i++) {
        if (definedRules && Object.keys(definedRules).length > 0) {
            ...
            if (ruleMatch) {
                ...
                if (rule.valueFunctionFile && rule.valueFunction) {
                    console.log(`Custom value function detected for ${name}...`);
                    const filePath = `${process.cwd()}/${rule.valueFunctionFile}`;
                    if (!fs.existsSync(filePath)) {
                        addError(
                            result, 
                            lineno, 
                            line, 
                            `Value function file not found at: ${filePath} for ${instruction}.${name}`);
                    } else {
                        const valueFunction = require(filePath)[rule.valueFunction];
                        const [validatorError, isValid] = valueFunction(stripQuotes(nameVals[name]));
                        if (!isValid) addResult(
                            result, 
                            lineno, 
                            line, 
                            validatorError.message, 
                            validatorError.level,
                            rule.reference_url);
                    }
                }
